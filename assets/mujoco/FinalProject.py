...
target_initial_pos = data.body('master_chef_can').xpos
...
th_tip_target = np.asarray([0.02039237, 0.05479632, 0.05815931])
...
cylinder_object_idx = model.body('master_chef_can').id
...
# Function that calculates the desired joint positions to move the hand towards the target.
def compute_angle_control_signals(kp,ki,kd,error):
    global integral_angle_error, prev_angle_error

    # Proportional term
    p = kp * error

    # Integral term
    integral_angle_error += error
    i = ki * integral_angle_error

    # Derivative term
    derivative_error = error - prev_angle_error
    d = kd * derivative_error
...
error_fingers = 0.025
error_joints = 0.001
angle_to_final_target = np.ones(16)
x=0
with mujoco.viewer.launch_passive(model, data) as viewer:
    # Close the viewer automatically after 30 wall-seconds.
    start = time.time()
    # data.ctrl shape 19, is the force for each joint
    # data.qpos shape 26, is joint, 0-3 is the root joint, 3-18 is the hand joint, 19-25 is the postion and orientation of the palm
    while viewer.is_running() and time.time() - start < 1000:
        step_start = time.time()
        # move the palm of Allegro hand to the place close to the cylinder
        if (np.abs(distance_to_target) > error).any() and init_flag == False:
          hand_initial_pos = data.body('palm').xpos

          # target_initial_pos = data.body('cylinder_object').xpos
          target_initial_pos = data.body('master_chef_can').xpos
          print("object pose ", target_initial_pos)
          print("hand pose", data.body('palm').xpos)
          # Calculate the distance to the target
          distance_to_target =  target_initial_pos - hand_initial_pos + [-0.025,0.01,0]
          ...
          print("approach objects")
        # move the fingertip postion to grasp the cylinder
        ...
        # if grasp_flag == False:
          ...
          if (np.abs(ff_tip_distance_to_target) < error_fingers).all():
            ff_flag = True
            
          if (np.abs(mf_tip_distance_to_target) < error_fingers).all():
            mf_flag = True
            
          if (np.abs(rf_tip_distance_to_target[0]) < error_fingers).all():
            rf_flag = True
          ...
          # print("ff_tip_distance_to_target ", ff_tip_distance_to_target)
          # print("mf_tip_distance_to_target ", mf_tip_distance_to_target)
          # print("rf_tip_distance_to_target ", rf_tip_distance_to_target)
          # print("th_tip_distance_to_target ", th_tip_distance_to_target)
          ...
          # print("control singnals: ", ff_tip_control_signals, mf_tip_control_signals, rf_tip_control_signals, th_tip_control_signals)
        ...
        # move the palm to a target pose
        ...
        elif (np.abs(angle_to_final_target) > error_joints).any():
          # get hand joint angles
          hand_initial_joints = data.qpos[3:19]
          hand_target_joints = np.zeros(16)
          hand_target_joints[12] = 0.46
          # hand_target_joints[13] = 0.9
          # hand_target_joints[14] = 0.8

          # embed()
          angle_to_final_target = hand_target_joints - hand_initial_joints
          # print("angle target: ", angle_to_final_target)

          data.ctrl[3:] = hand_target_joints
          # print(np.round(data.qpos[3:19], 2))

          # for j in range(12):
          #   data.ctrl[j+3] = compute_angle_control_signals(0.02,0,0,angle_to_final_target[j])
          # data.ctrl[15] = compute_angle_control_signals(0.0002,6000,1000,angle_to_final_target[12])
          # print("ddddda:", np.round(angle_to_final_target[12], 2))
          # for j in range(3):
          #   data.ctrl[16+j] = compute_angle_control_signals(0.02,0,0,angle_to_final_target[13+j])
